...
styledstring styledstring::operator+(styledstring&& sstr) const noexcept {
  ...
  std::move(
      sstr.attrs.begin(), sstr.attrs.end(), std::back_inserter(tmp.attrs));
  return tmp;
}
...
styledstring& styledstring::operator+=(styledstring const& sstr) & noexcept {
  ...
  std::move(
      std::begin(tmp.attrs), std::end(tmp.attrs), std::back_inserter(attrs));
  return *this;
}
...
void styledstring::bold() noexcept {
  bold(range{0, content.size()});
}
void styledstring::italic() noexcept {
  italic(range{0, content.size()});
}
void styledstring::underline() noexcept {
  underline(range{0, content.size()});
}
...
void styledstring::trim() noexcept {
  ...
  if (l) {
    ...
    for (auto it = std::rbegin(attrs);
         it->pos.finish > size && it != std::rend(attrs);
         ++it) {
      it->pos.finish -= l;
